...
<script >
	...
	export default {
		...
		onPullDownRefresh() {
			...
			this.getQuestionReplyList(this.nid, 1, false, stopRefresh);
		},
		components:{
			uIcon,
			uReadMore,
		...
		onLoad(option) {
			console.log('onLoad...'+this.nowpage+',nid:'+this.nid+',pageNo:'+this.pageNo);
			this.nid = option.nid;
		...
		methods: {
			...
			init(nid) {
				...
				this.getQuestionReplyList(nid, this.pageNo);
			...
			async getQuestionReplyList(nid,pageNo,isApppend, stopRefresh) {
				const data = getApp().globalData;
				const apiPrefix = data.serverUri;
				const auth = data.auth;
				const params = {
			...
			lower() {
				// 滑动到最下面时候触发
				let _self = this;
				console.log('page',this.nowpage, 'totalpage', this.totalpage);
				...
				this.interval = setTimeout(function() {
					this.nowpage++;
					console.log('page....',this.nowpage);
					_self.getQuestionReplyList(this.nid, this.nowpage, 1);
				}, 500);
			},
			publish() {
				if(!this.uid) {
